...
object SublistSpecs {
  ...
  def transitivity[T](l1: List[T], l2: List[T], l3: List[T]): Unit = {
    ...
    (l1, l2, l3) match {
        case (Cons(h1,t1), Cons(h2,t2), Cons(h3,t3)) => {
          if (sublist(l2,t3)) { 
            transitivity(l1, l2, t3)
          } else if (sublist(l1, t2)) {
            transitivity(l1, t2, t3)
          } else {
  ...
  def lengthHomomorphism[T](l1: List[T], l2: List[T]): Unit = {
    ...
    (l1, l2) match {
        case (Cons(h1,t1), Cons(h2,t2)) => {
          if (sublist(l1,t2)) {
            lengthHomomorphism(l1,t2)
          } else {
            lengthHomomorphism(t1,t2)
          }
        }
        case _ => ()
    }
  ...
  */
  ...
  def append[T](l1: List[T], l2: List[T], l: List[T]): Unit = {
    ...
    (l1, l2) match {
        ...
        case (Nil(), Cons(h2, t2)) => append(l1, t2, l)
        case (_, Nil()) => reflexivity(l)
    }
  }.ensuring(_ =>
    sublist(l1 ++ l, l2 ++ l)  
  )
